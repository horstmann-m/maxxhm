...
The CRM holds personal data of real clients and suppliers (names, emails, phone numbers, notes, business-card photos), so it's treated as in-scope for GDPR regardless of being solo-operated.

- **Lawful basis**: Art. 6(1)(f) legitimate interest — B2B contact data processed for an existing or prospective trading relationship, the standard basis for a trade CRM. No marketing use beyond direct trading correspondence is in scope; if that changes (e.g. bulk marketing emails), consent (Art. 6(1)(a)) would need to be captured separately.
- **Transparency (Art. 13/14)**: contacts added via CSV import or business-card capture haven't given the CRM operator anything directly — they're informed at first outreach (email/WhatsApp) that their details are held for the trading relationship, consistent with legitimate-interest processing of B2B contacts.
- **Security of processing (Art. 32)**: every API route requires a valid JWT bearer token issued via `/auth/login`, checked on every request by a global `onRequest` hook (`backend/src/lib/auth.ts`); the one necessary exception, the Twilio WhatsApp webhook, is protected instead by Twilio's own request-signature verification rather than being left open. The frontend never lets a browser hold the JWT directly — it's stored in an httpOnly cookie set by the Next.js server, and all client-side mutations go through same-origin `/api/*` proxy routes that attach it server-side (`frontend/src/middleware.ts`, `frontend/src/lib/session.ts`).
- **Right to erasure (Art. 17)**: `DELETE /contacts/:id` performs a real delete, not a soft-delete flag — the Prisma schema cascades to that contact's conversations, messages, activities, and deals (`onDelete: Cascade`), so erasure is complete. Exposed in the UI as "Delete contact & all data" on the contact detail page.
- **Right of access & portability (Art. 15/20)**: `GET /contacts/:id/export` returns the full record (contact + activities + deals + conversations/messages) as a downloadable JSON document. Exposed in the UI as "Export data (JSON)" on the contact detail page.
- **Retention**: no automatic deletion job exists yet. The dormancy thresholds already used for follow-up nudges (§6 — 60 days clients / 120 days suppliers) are a reasonable trigger point to periodically review and manually purge contacts that never converted, but this isn't automated — flagged as a Phase 2 candidate rather than implemented now, since automatic deletion of a real business relationship's data needs a human decision, not a silent cron job.
- **Sub-processors** (Art. 28 — each needs a DPA before production use):
  - Hetzner Online GmbH (hosting, DB, object storage) — EU-only.
  - Mistral AI (business-card OCR) — EU-only.
  - Microsoft (M365/Graph email) — accepted exception, §14.
  - Twilio / Meta (WhatsApp) — accepted exception, §14.
- **Breach notification**: no incident-response process exists yet beyond "this is a solo operation" — a documented breach-notification checklist (72h authority notification per Art. 33) should exist before go-live; this is a process to write, not something implementable in code.
